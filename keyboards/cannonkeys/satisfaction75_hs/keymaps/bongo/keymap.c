// Copyright 2023 Andrew Kannan (@awkannan)
// SPDX-License-Identifier: GPL-2.0-or-later

#include QMK_KEYBOARD_H
#include "kirby.h"

enum custom_keycodes {
    WIN_CLIP_BOARD = SAFE_RANGE,
    WIN_SCREENSHOT,
}

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        // カービィ ジャンプアニメーション用
        case KC_SPC:
            if (record->event.pressed) {
                if (!prev_jump_state) {
                    is_jump = true;
                } else {
                    is_jump = false;
                }

                prev_jump_state = true;
            } else {
                is_jump         = false;
                prev_jump_state = false;
            }

            return true;
        case WIN_CLIP_BOARD:
            if (record->event.pressed) {
                SEND_STRING(SS_LGUI("v"));
                return false;
            }
            return true;
        case WIN_SCREENSHOT:
            if (record->event.pressed) {
                SEND_STRING(SS_LGUI(SS_LSFT("s")));
                return false;
            }
            return true;
        default:
            return true;
    }
}

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT_all(
    KC_ESC,  KC_F1,   KC_F2,   KC_F3, KC_F4, KC_F5,  KC_F6, KC_F7, KC_F8, KC_F9,   KC_F10,  KC_F11,  KC_F12,  ENC_PRESS,
    KC_F13,  KC_1,    KC_2,    KC_3,  KC_4,  KC_5,   KC_6,  KC_7,  KC_8,  KC_9,    KC_0,    KC_MINS, KC_EQL,  KC_BSPC,   XXXXXXX,
    KC_TAB,  KC_Q,    KC_W,    KC_E,  KC_R,  KC_T,   KC_Y,  KC_U,  KC_I,  KC_O,    KC_P,    KC_LBRC, KC_RBRC, KC_BSLS,   KC_GRV,
    KC_LCTL, KC_A,    KC_S,    KC_D,  KC_F,  KC_G,   KC_H,  KC_J,  KC_K,  KC_L,    KC_SCLN, KC_QUOT,          KC_ENTER,  KC_DEL,
    KC_LSFT,          KC_Z,    KC_X,  KC_C,  KC_V,   KC_B,  KC_N,  KC_M,  KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT, KC_UP,     XXXXXXX,
    MO(1),   KC_LALT, KC_LALT,               KC_SPC,                      KC_RGUI, MO(1),   KC_RALT, KC_LEFT, KC_DOWN,   KC_RGHT
  ),
  [1] = LAYOUT_all(
    _______, _______, _______,        _______, _______, _______,        _______, _______, _______, _______, _______, _______, _______, _______,
    _______, _______, _______,        _______, _______, _______,        _______, _______, _______, _______, _______, _______, _______, _______, _______,
    _______, _______, _______,        _______, _______, _______,        _______, _______, _______, _______, _______, _______, _______, _______, CLOCK_SET,
    _______, _______, WIN_SCREENSHOT, _______, _______, _______,        _______, _______, _______, _______, _______, _______,          _______, OLED_TOGG,
    _______,          _______,        _______, _______, WIN_CLIP_BOARD, _______, _______, _______, _______, _______, _______, _______, _______, _______,
    _______, _______, _______,                          _______,                                   _______, _______, _______, KC_HOME, _______, KC_END
  ),
  [2] = LAYOUT_all(
    _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
    _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
    _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
    _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,          _______, _______,
    _______,          _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
    _______, _______, _______,                   _______,                            _______, _______, _______, _______, _______, _______
  )
};
